...
var gTier2LastFullRefreshStartSecondPixel;

var gTier3StartSeconds;
var gTier3LastOnFrameStartGETSeconds;
var gTier3LastFullRefreshStartSeconds;
...
function initNavigator() {
  ...
  //init navigator
  gCurrMissionTime = timeIdToTimeStr(cDefaultStartTimeId); //set clock to start time; //TODO make this handle t parameter time

  gTier1Group = new paper.Group();
  gTier1NavGroup = new paper.Group();
  gTier2Group = new paper.Group();
  gTier2BoarderGroup = new paper.Group();
  gTier2NavGroup = new paper.Group();
  gTier3Group = new paper.Group();
  gTier3BoarderGroup = new paper.Group();
  ...
  paper.view.onMouseUp = function (event) {
    ...
    if (event.point.y < gTier1Top + gTier1Height + gTierSpacing) {
      ...
      //'send', 'event', 'navigator', 'click', 'tier1');
      mouseXSeconds = (event.point.x - gTier1Left) * gTier1SecondsPerPixel - cCountdownSeconds;
    } else if (
      event.point.y >= gTier1Top + gTier1Height + gTierSpacing &&
      event.point.y < gTier2Top + gTier2Height + gTierSpacing
...
function setDynamicWidthVariables() {
  ...
  gTier1PixelsPerSecond = gTier1Width / (cMissionDurationSeconds + cCountdownSeconds);
  gTier1SecondsPerPixel = (cMissionDurationSeconds + cCountdownSeconds) / gTier1Width;
  gTier2PixelsPerSecond =
    gTier2Width / ((cMissionDurationSeconds + cCountdownSeconds) / gNavZoomFactor);
  gTier2SecondsPerPixel =
    (cMissionDurationSeconds + cCountdownSeconds) / gNavZoomFactor / gTier2Width;
  gTier3PixelsPerSecond =
    gTier3Width / ((cMissionDurationSeconds + cCountdownSeconds) / gNavZoomFactor / gNavZoomFactor);
  gTier3SecondsPerPixel =
    (cMissionDurationSeconds + cCountdownSeconds) / gNavZoomFactor / gNavZoomFactor / gTier3Width;
}
...
function drawCursor(seconds) {
  ...
  var cursorLocX = gTier1Left + (seconds + cCountdownSeconds) * gTier1PixelsPerSecond;
...
function drawTier1() {
  ...
  for (var i = 0; i < gMissionStages.length; i++) {
    var itemLocX =
      gTier1Left +
      (timeStrToSeconds(gMissionStages[i][0]) + cCountdownSeconds) * gTier1PixelsPerSecond;
  ...
  for (i = 0; i < gVideoSegments.length; i++) {
    var rectStartX =
      gTier1Left +
      (timeStrToSeconds(gVideoSegments[i][0]) + cCountdownSeconds) * gTier1PixelsPerSecond;
    var rectWidth =
      (timeStrToSeconds(gVideoSegments[i][1]) - timeStrToSeconds(gVideoSegments[i][0])) *
  ...
  for (i = 0; i < gPhotoData.length; i++) {
    if (gPhotoData[i][0] !== "") {
      itemLocX =
        gTier1Left +
        (timeIdToSeconds(gPhotoData[i][0]) + cCountdownSeconds) * gTier1PixelsPerSecond;
  ...
  for (i = 0; i < gOrbitData.length; i++) {
    itemLocX =
      gTier1Left + (timeStrToSeconds(gOrbitData[i][0]) + cCountdownSeconds) * gTier1PixelsPerSecond;
...
function drawTier1NavBox(seconds) {
  ...
  gTier2StartSeconds = gTier1SecondsPerPixel * (gTier1NavBoxLocX - gTier1Left) - cCountdownSeconds;
...
function drawTier2(forceRefresh) {
  ...
  // trace('gTier2StartSeconds: ' + gTier2StartSeconds * gTier2PixelsPerSecond + " gTier2LastFullRefreshStartSeconds: " + gTier2LastFullRefreshStartSecondPixel);
  if (
    forceRefresh ||
    Math.round(gTier2StartSeconds * gTier2PixelsPerSecond) !== gTier2LastFullRefreshStartSecondPixel
  ) {
    gTier2LastFullRefreshStartSecondPixel = Math.round(gTier2StartSeconds * gTier2PixelsPerSecond);

    gTier2BoarderGroup.removeChildren();
    ...
    gTier2BoarderGroup.addChild(tierRectPath);
    ...
    for (var i = 0; i < gVideoSegments.length; i++) {
      ...
      if (
        timeStrToSeconds(gVideoSegments[i][0]) <= gTier2StartSeconds + secondsOnTier2 &&
        timeStrToSeconds(gVideoSegments[i][1]) >= gTier2StartSeconds
      ) {
        var rectStartX =
          gTier2Left +
          (timeStrToSeconds(gVideoSegments[i][0]) - gTier2StartSeconds) * gTier2PixelsPerSecond;
        var rectWidth =
          (timeStrToSeconds(gVideoSegments[i][1]) - timeStrToSeconds(gVideoSegments[i][0])) *
          gTier2PixelsPerSecond;
        if (rectStartX < 0) {
    ...
    for (i = 0; i <= gOrbitData.length - 1; i++) {
      ...
      if (
        timeStrToSeconds(gOrbitData[i][0]) <= gTier2StartSeconds + secondsOnTier2 &&
        timeStrToSeconds(gOrbitData[i][2]) >= gTier2StartSeconds
      ) {
        itemLocX =
          gTier2Left +
          (timeStrToSeconds(gOrbitData[i][0]) - gTier2StartSeconds) * gTier2PixelsPerSecond;
    ...
    for (i = 0; i <= gMissionStages.length - 1; i++) {
      ...
      if (
        timeStrToSeconds(gMissionStages[i][0]) <= gTier2StartSeconds + secondsOnTier2 &&
        timeStrToSeconds(gMissionStages[i][3]) >= gTier2StartSeconds
      ) {
        itemLocX =
          gTier2Left +
          (timeStrToSeconds(gMissionStages[i][0]) - gTier2StartSeconds) * gTier2PixelsPerSecond;
...
function drawTier3(forceRefresh, drawForwardForScrolling) {
  ...
  if (drawForwardForScrolling) {
    var lengthMultiplier = 2;
    // trace("lengthMultiplier 2")
  } else {
    lengthMultiplier = 1;
    // trace("lengthMultiplier 1")
  }
  ...
  if (forceRefresh || gTier3StartSeconds > gTier3LastFullRefreshStartSeconds + secondsOnTier3) {
    ...
    gTier3BoarderGroup.removeChildren();
    ...
    // tempGroup.addChild(tierRectPath);
    gTier3BoarderGroup.addChild(tierRectPath);
    // gTier3BoarderGroup.sendToBack();
    ...
    for (var i = 0; i <= gVideoSegments.length - 1; i++) {
      ...
      if (
        timeStrToSeconds(gVideoSegments[i][0]) <=
          gTier3StartSeconds + secondsOnTier3 * lengthMultiplier &&
        timeStrToSeconds(gVideoSegments[i][1]) >= gTier3StartSeconds
      ) {
        var rectStartX =
          gTier3Left +
          (timeStrToSeconds(gVideoSegments[i][0]) - gTier3StartSeconds) * gTier3PixelsPerSecond;
        var rectWidth =
          (timeStrToSeconds(gVideoSegments[i][1]) - timeStrToSeconds(gVideoSegments[i][0])) *
          gTier3PixelsPerSecond;
        if (rectStartX < 0) {
        ...
        if (rectWidth > gTier3Width * 2 - rectStartX - 1) {
          rectWidth = gTier3Width * 2 - rectStartX + gTier3Left - 3;
        }
    ...
    for (i = 0; i < missionDurationHours * 2; i++) {
      ...
      if (itemSecondsFromLeft > secondsOnTier3 * lengthMultiplier) break;
      if (itemSecondsFromLeft >= 0) {
    ...
    for (i = 0; i <= gOrbitData.length - 1; i++) {
      ...
      if (
        ...
        itemLocX =
          gTier3Left +
          (timeStrToSeconds(gOrbitData[i][0]) - gTier3StartSeconds) * gTier3PixelsPerSecond;
    ...
    for (i = 0; i <= gMissionStages.length - 1; i++) {
      ...
      if (
        ...
        itemLocX =
          gTier3Left +
          (timeStrToSeconds(gMissionStages[i][0]) - gTier3StartSeconds) * gTier3PixelsPerSecond;
    ...
    for (i = 0; i < gTOCData.length; i++) {
      ...
      if (itemSecondsFromLeft >= 0 && itemSecondsFromLeft <= secondsOnTier3 * lengthMultiplier) {
        itemLocX = gTier3Left + itemSecondsFromLeft * gTier3PixelsPerSecond;
