    ...
    if (data[0] !== "") {
      ...
      if (data[2].length === 0) {
        data[1] = data[1].replace(
          "ALSJ",
          '<a href="https://www.hq.nasa.gov/alsj/a11/a11.html" target="alsj">ALSJ</a> Commentary'
        );
        data[1] = data[1].replace(
          "AFJ",
      ...
      data[3] = data[3].replace(/Tig /gi, "T<sub>ig</sub> ");
      data[3] = data[3].replace(/DELTA-VC/gi, "DELTA-V<sub>c</sub>");
      data[3] = data[3].replace(/DELTA-VT/gi, "DELTA-V<sub>t</sub>");
...
function createSearchData() {
  ...
  for (var counter = 0; counter < gUtteranceData.length; counter++) {
    ...
    tmpItem[3] = gUtteranceData[counter][2].replace(/(<([^>]+)>)/gi, ""); //strip HTML tags
    tmpItem[4] = 0;
