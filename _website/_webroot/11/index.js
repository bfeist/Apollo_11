...
function seekToTime(timeId) {
  ...
  for (var i = 0; i < gMediaList.length; ++i) {
    ...
    if (totalSeconds >= itemStartTimeSeconds && totalSeconds < itemEndTimeSeconds) {
...
function scrollCommentaryToTimeId(timeId) {
  ...
  if (gCommentaryDataLookup.hasOwnProperty(timeId)) {
    ...
    } else if (gCommentaryDataLookup[timeId] < gCommentaryDisplayStartIndex + 49) {
      //prepend
    ...
    } else if (gCommentaryDataLookup[timeId] > gCommentaryDisplayEndIndex - 49) {
      //append
...
function prependUtterances(count, atTop) {
  ...
  if (atTop) {
    ...
    var oldScrollDestination =
      utteranceDiv.scrollTop() + elementToScrollBackTo.offset().top - utteranceDiv.offset().top;
  ...
  for (var i = 0; i < gGeoData.length; i++) {
    if (gGeoData[i][0] === utteranceObject[0]) {
      ...
      words_modified = words_modified.replace(
        re,
        "<a href='javascript:;' onclick='loadGeosampleIntoOverlay(" +
          i +
          ");'>" +
          gGeoData[i][3] +
          "</a>"
...
function prependCommentary(count, atTop) {
  ...
  if (gCommentaryDisplayStartIndex > 0) {
    ...
    if (atTop) {
      ...
      var oldScrollDestination =
        commentaryDiv.scrollTop() + elementToScrollBackTo.offset().top - commentaryDiv.offset().top;
...
function populatePhotoGallery() {
  ...
  for (var i = 0; i < gPhotoIndex.length; i++) {
    ...
    if (filetypematch !== null) {
      ...
      var imageURL =
        cLPICdnRoot + "/resources/apollo/images/thumb/AS11/" + rollNum + "/" + imgNum + ".jpg";
    } else if (photoObject[3] !== "") {
      //if custom image URL in datafile
...
function showPhotoByTimeId(timeId) {
  ...
  if (gPhotoDataLookup.hasOwnProperty(timeId)) {
    ...
    var scrollDest =
      photoGalleryDiv.scrollTop() + $(photoGalleryImageTimeId).offset().top - gNavigatorHeight - 70; //added offset for ted design
...
function loadPhotoHtml(photoIndex) {
  ...
  if (filetypematch !== null) {
    ...
    if (photoObject[2] !== "") {
      ...
      imageURL =
        cLPICdnRoot + "/resources/apollo/images/print/AS11/" + rollNum + "/" + imgNum + ".jpg";
    }
  } else if (photoObject[3] !== "") {
    //if custom image URL in datafile
    imageURL = photoObject[3];
...
function updateDashboard(timeId) {
  ...
  if (timeIdInSeconds < timeStrToSeconds("075:49:44")) {
    //trans-lunar coast
    calculateVelocity = true;
    calculateDistanceFromEarth = true;
  } else if (timeIdInSeconds > timeStrToSeconds("135:23:36")) {
    //trans-earth coast
    calculateVelocity = true;
    calculateDistanceFromEarth = true;
  } else {
  ...
  if (calculateVelocity) {
    if (timeIdInSeconds < timeStrToSeconds("195:18:18")) {
      ...
      var currentVelocityFPS =
        (currentPositionInSecondsRange * velocityRange) / secondsRange + startVelocity;
    } else {
    ...
    var dashVelocity =
      ...
      '</span> km/h, <span class="value">' +
      numberWithCommas(currentVelocityMPH) +
      '</span> mph, Mach <span class="value">' +
      currentVelocityMach +
      "</span>)";
  ...
  if (calculateDistanceFromEarth) {
    if (timeIdInSeconds > 0 && timeIdInSeconds < timeStrToSeconds("195:18:18")) {
      ...
      var currentDistanceEarthNM =
        (currentPositionInSecondsRange * distanceEarthRange) / secondsRange + startDistanceEarth;
    } else {
...
function manageOverlaysAutodisplay(timeId) {
  ...
  for (var counter = 0; counter < gVideoSegments.length; counter++) {
    if (
      ...
      if (
        dashboardOverlay.css("display").toLowerCase() !== "none" &&
...
function openGeosampleOverlay() {
  ...
  for (var i = 0; i < gGeoData.length; i++) {
    ...
    samplerows +=
      "<tr onclick='seekToGeosample(" +
      i +
      ")'>" +
      "<td>" +
      timeIdToTimeStr(gGeoData[i][0]) +
      "</td>" +
      "<td class='samplecontainerslink'>" +
      gGeoData[i][2] +
      "</td>" +
      "<td>" +
      sampleNumsItem +
    ...
    for (var paperCounter = 0; paperCounter < gPaperData.length - 1; paperCounter++) {
      if (gPaperData[paperCounter][10].includes(sampleNumberArray[counter])) {
        ...
        if (gPaperData[paperCounter][5] !== "")
          journalString = journalString + ", Vol. " + gPaperData[paperCounter][5];
        if (gPaperData[paperCounter][6] !== "")
          journalString = journalString + ", Issue " + gPaperData[paperCounter][6];
        if (gPaperData[paperCounter][7] !== "")
          journalString = journalString + ", Page " + gPaperData[paperCounter][7];
        ...
        if (gPaperData[paperCounter][9] !== "") {
          ...
          linkURL =
            "https://ui.adsabs.harvard.edu/#abs/" + gPaperData[paperCounter][0] + "/abstract";
        } else {
    ...
    for (
      ...
      if (gGeoCompendiumData[compendiumCounter][1].includes(sampleNumberArray[counter])) {
        ...
        if (gGeoCompendiumData[compendiumCounter][2] === "compendium") {
          compendiumHtml =
            '<span> - <a href="https://curator.jsc.nasa.gov/lunar/lsc/' +
            compendiumSampleNumber +
            '.pdf" target="geoImage">Lunar Sample Compendium (PDF)</a></span>';
        } else {
          compendiumHtml =
            '<span> - <a href="https://curator.jsc.nasa.gov/lunar/catalogs/apollo11/' +
            compendiumSampleNumber +
            '.pdf" target="geoImage">Lunar Sample Information Catalog (PDF)</a></span>';
        }
        break;
      }
    }
...
function proportionalWidthOnPhotoBlock() {
  var appBlockWidth =
    $("body").width() - ($(".video-block").width() + $(".thirtytrack-block").width()) - 1;
...
function thirtyButtons_click() {
  ...
  if (gMOCRToggled === false) {
    ...
    //MOCRviz already open so just change channel
    var MOCRvizIframeSelector = $("#MOCRvizIframe")[0];
...
function getTapeActivityRanges(activeSec) {
  ...
  if (nearestEnd > cMissionDurationSeconds + cCountdownSeconds) {
  ...
  var tapesActivityfilename =
    "tape_activity_" + nearestStart.toString() + "-" + (endRange - 1).toString() + ".json";
