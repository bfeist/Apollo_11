...
inputFilePath = "E:/Apollo_13_Data_Delivery/Apollo13/"
csv.register_dialect('pipes', delimiter='_', doublequote=True, escapechar='\\')
reader = csv.reader(open(inputFilePath + "wav_files", "rU"), dialect='pipes')
...
for row in reader:
    if row[1] != lasttape:
        ...
        if os.path.exists(outputPath + outputDirectoryName + outputWavFilename):
    ...
    outputFilelist.write("file " + inputFilePath + "_".join(map(str, row)) + "'\n")
